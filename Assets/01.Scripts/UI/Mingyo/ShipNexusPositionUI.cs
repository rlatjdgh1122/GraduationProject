...
public class ShipNexusPositionUI : MonoBehaviour
{
    ...
    private Vector3 _nexusScreenPos
    {
        ...
        {
            // 선박의 위치를 화면 좌표로 변환
            Vector3 pos = Camera.main.WorldToScreenPoint(_nexusPos);

            // 화면 내에서 선박 위치를 유지시키기 위해 제한을 둠
            return new Vector2(Mathf.Clamp(pos.x - 10, 50f, Screen.width - 50f),
    ...
    private void LateUpdate()
    {
        ...
        _nexusbutton.gameObject.rectTransform().position = _nexusScreenPos; //배UI위치를 배 위치로
    }
}
